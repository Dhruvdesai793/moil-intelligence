...
def test_postgis_repository_roundtrip():
    ...
    with get_engine().connect() as connection:
        ...
        with Session(bind=connection, join_transaction_mode="create_savepoint") as session:
            ...
            session.add(ExplorationSite(
                id=site_id, name="Integration test fixture", status="ACTIVE", region="Test",
                latitude=21, longitude=79, geometry=WKTElement("POINT(79 21)", srid=4326)))
            session.flush()
            site = ExplorationRepository(session).get(site_id)
            assert site.latitude == 21 and site.status == "ACTIVE"
            srid = session.scalar(text("SELECT ST_SRID(geometry) FROM exploration_sites WHERE id=:id"),
                                  {"id": site_id})
            assert srid == 4326
            bundle = FeatureBundle(feature_id=str(uuid4()), site_id=site_id,
                                   coordinates=Coordinates(latitude=21, longitude=79),
                                   feature_version="test-v1", source="demo_fixture",
                                   is_stub=True, readiness="stub", extracted_at=utc_now(),
            ...
            assert features.get_latest_for_site(site_id, bundle.extracted_at - timedelta(days=1)) is None
            assert len(features.list_for_site(site_id)) == 1
            prediction = ExplorationPrediction(
                prediction_id=str(uuid4()), site_id=site_id, coordinates=bundle.coordinates,
                model_version="stub-v1", feature_version=bundle.feature_version, as_of=utc_now(),
                models=[], is_stub=True)
            predictions = PredictionRepository(session)
            predictions.save(prediction)
            assert predictions.get(prediction.prediction_id).is_stub
            assert any(r.prediction_id == prediction.prediction_id for r in predictions.list_recent())
            job = JobRecord(job_id=str(uuid4()), status="queued", request=JobRequest(site_id=site_id))
