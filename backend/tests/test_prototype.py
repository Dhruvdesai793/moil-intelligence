...
from app.api.dependencies import (
    get_db, get_exploration_repository, get_feature_repository, get_prediction_repository,
    get_job_repository, get_registry, get_gee, get_health,
)
...
def client():
    ...
    app.dependency_overrides.update({
        ...
        get_gee: lambda: gee,
        get_health: lambda: HealthService(lambda: (_ for _ in ()).throw(RuntimeError()),
                                          gee, registry, Settings(_env_file=None)),
    })
...
def test_missing_or_broken_models(adapters):
    ...
    exploration = ExplorationService(FakeExplorationRepository(), registry, gee, WeatherProvider())
    features = FeatureService(FakeFeatures(), exploration, gee, settings)
    orchestrator = PredictionOrchestrator(exploration, registry, FakeRecords(), features)
...
def test_features_require_explicit_demo_fallback(client):
    payload = {"site_id": "zone_a", "start_date": "2025-01-01", "end_date": "2025-02-01"}
    result = client.post("/api/v1/features/extract", json=payload).json()
    ...
    prediction = client.post("/api/v1/predictions/exploration", json={"site_id": "zone_a"}).json()
    ...
    assert datetime.fromisoformat(prediction["data_timestamp"]) <= datetime.fromisoformat(prediction["as_of"])
...
def test_feature_aoi_demo(client):
    result = client.post("/api/v1/features/extract", json={
        "aoi": {"type": "Polygon", "coordinates": [[[79, 21], [80, 21], [80, 22], [79, 21]]]},
        "start_date": "2025-01-01", "end_date": "2025-02-01", "allow_demo_fallback": True,
    }).json()
    assert result["is_stub"] is True
...
def test_historical_prediction_does_not_read_future_features(client):
    client.post("/api/v1/features/extract", json={
        "site_id": "zone_a", "start_date": "2025-01-01", "end_date": "2025-02-01",
        "allow_demo_fallback": True,
    })
    result = client.post("/api/v1/predictions/exploration", json={
        "site_id": "zone_a", "as_of": "2025-03-01T00:00:00Z",
    }).json()
...
def test_gee_initialization_order_and_cache(monkeypatch):
    ...
    assert provider.availability().readiness == "ok"
    assert calls == [("initialize", "secure-guru-473417-q2"), ("deadline", 5000), ("smoke", None)]
...
def test_prediction_read_and_feature_history(client):
    ...
    assert client.get("/api/v1/predictions/" + prediction["prediction_id"]).json() == prediction
    ...
    for path in directory.glob("*.py"):
        for node in ast.walk(ast.parse(path.read_text())):
            if isinstance(node, ast.Call):
                ...
                assert not name.endswith(("Repository", "Provider", "Orchestrator")), str(path)
                assert name not in ("create_engine", "Session", "SessionLocal"), str(path)
