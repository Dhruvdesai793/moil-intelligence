...
class DecisionService:
    ...
    def recommendations(self) -> Recommendations:
        ...
        return Recommendations(recommendations=[
            Recommendation(
                priority=Priority.HIGH if overview.risk == Risk.HIGH else Priority.NORMAL,
                actions=["Review the demo production schedule with a human planner.",
                         "Obtain validated operational history before making mine decisions."],
                drivers=[f"Demo production risk: {overview.risk.value}",
                         f"Demo active exploration sites: {len(sites)}"],
            ),
            Recommendation(priority=Priority.NORMAL,
                           actions=["Validate exploration inputs and baseline before field action."],
                           drivers=["GEE features and scientifically validated models unavailable"]),
