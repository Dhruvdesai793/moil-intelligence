...
class FeatureService:
    ...
    def get(self, feature_id):
        result = self.repository.get(feature_id)
        if result is None:
            raise NotFoundError(f"Feature bundle '{feature_id}' was not found.")
        return result
    ...
    def extract(self, request: FeatureExtractionRequest) -> FeatureBundle:
        ...
        if (
            result.readiness != Readiness.OK
            and request.allow_demo_fallback
            and self.settings.gee_allow_demo_features
        ):
            result = result.model_copy(
                update={
                    ...
                    "feature_payload": {
                        "integration_fixture_value": 0.5,
                        "purpose": "software_contract_test",
                    },
                    "spectral_bands": [],
                    "quality": {},
                    "extraction_method": "Deterministic software integration fixture; not Earth Engine extraction.",
