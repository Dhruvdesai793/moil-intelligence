...
from app.schemas.exploration import Site, SiteSummary
...
def get_site(site_id: str, service: ExplorationService = Depends(get_exploration)) -> Site:
    return service.get_site(site_id)


@router.get("/sites/{site_id}/summary", response_model=SiteSummary)
def get_summary(site_id: str, service: ExplorationService = Depends(get_exploration)) -> SiteSummary:
